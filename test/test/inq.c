...
"));
			...
			for (i = 0; i < buf[3]; i++) {
				debug && printf("@ %8lu:%06lu ... ", resultime.tv_sec, resultime.tv_usec);
				printf("RSSSI CAJHNDFA %d", (inquiry_info_with_rssi*)&buf[4+i*sizeof(inquiry_info_with_rssi)]);
...
char *envp[];
{
	int opt,dd,iq,length = 0x08,dongle_n = -1, fd = -1;
	char *str = "OK\r\n";

	while ((opt=getopt_long(argc, argv, "+i:hdl:p", long_options, NULL)) != -1) {
		switch(opt) {
		case 'i':
			dongle_n = hci_devid(optarg);
			if (dongle_n < 0) {
				fprintf(stderr, "That dongle does not exist!\n");
				return EXIT_FAILURE;
			}
			break;;
		case 'p':
