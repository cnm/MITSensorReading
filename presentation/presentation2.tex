...
\pgfdeclareimage[height=6cm]{arch_gsd_plat}{figures/gsd_platform_new.pdf}
...
\pgfdeclareimage[height=6.6cm]{ir_proto}{figures/ir_proto_new.jpg}
\pgfdeclareimage[height=6.6cm]{ir_emitters}{figures/ir_emitters_new.jpg}
\pgfdeclareimage[height=6.6cm]{ir_entrance1}{figures/ir_entrance1.jpg}
\pgfdeclareimage[height=6.6cm]{ir_entrance2}{figures/ir_entrance2.jpg}
\pgfdeclareimage[height=6.6cm]{ir_receivers}{figures/ir_receivers_new.jpg}
\pgfdeclareimage[height=6.6cm]{bt_proto}{figures/bt_simple_new.jpg}

%TESTS
\pgfdeclareimage[height=4cm]{test_mov}{figures/movimento_new.pdf}
\pgfdeclareimage[height=3.7cm]{test1.2}{figures/13-16zona2.pdf}
\pgfdeclareimage[height=3.7cm]{test1.3}{figures/13-16zona3.pdf}
\pgfdeclareimage[height=3.7cm]{test2.2}{figures/17-20zona2.pdf}
\pgfdeclareimage[height=3.7cm]{test2.3}{figures/17-20zona3.pdf}
\pgfdeclareimage[height=3.7cm]{test3.2}{figures/21-23zona2.pdf}
\pgfdeclareimage[height=3.7cm]{test3.3}{figures/21-23zona3.pdf}
...
\begin{frame}
  ...
  \begin{itemize}\setlength{\itemsep}{7mm}
	\item<1-> Dificuldade de gerir e reduzir consumos energéticos num campus
...
\begin{frame}
  ...
  \begin{itemize}\setlength{\itemsep}{7mm}
	...
	\item<2-> Utilizar plataforma multi-funcional de monitorização
	\item<3-> Integrar na plataforma supra-mencionada módulo de descoberta de serviços
  \end{itemize}
\end{frame}
\note[enumerate]       % Add notes to yourself that will be displayed when
{                      % typeset with the notes or notesonly class options
...
\framesubtitle{Vantagens de uso da plataforma}   % Insert frame title between curly braces
  ...
  \begin{itemize}\setlength{\itemsep}{8mm}
 		...
 		\item<2-> Lidar com requisitos de determinadas aplicações ao nível de rede como a sincronização
			...
			\item<4-> Possibilidade de inserir novos módulos que lidem com outro tipo de requisitos como compressão e cifra
...
\begin{frame}
  ...
  \item<2-> Adaptável a edifícios ou espaços de diferentes dimensões
  \item<3-> Flexível quanto às tecnologias  % Use Next Page to go to Point 3
  \item<4-> Baixa complexidade de computação
  \item<5-> Respeito pela privacidade
  \end{itemize}
\end{frame}
\note{Speak clearly}  % Add notes to yourself that will be displayed when
                      % typeset with the notes or notesonly class options
  ...
  \begin{columns}
   ...
   \column{.5\textwidth}
        \begin{itemize}\setlength{\itemsep}{2mm}
	  	...
	  	\item<2-> \emph{Spotters} %estão associados a zonas e interagem com sensores
		\item<3-> \emph{Zone Managers} %realizam a computação de localização e contagem na zona
		\item<4-> \emph{Aggregator} %é o nó sink que agrega a informação de localização de todas as zonas
		\item<5-> Servidor \emph{UMP} %armazena a informação ao longo do tempo e calcula padrões de movimento
		%\item<2-> Nós espalhados pela área de monitorização
	  	%\item<3-> Nós contêm diferentes tipos de sensores  % Use Next Page to go to Point 3
	  	%\item<4-> Informação é agrupada e enviada para um servidor para armazenamento
        \end{itemize}
...
\begin{frame}
  ...
  \begin{columns}
   ...
   \column{.5\textwidth}
        ...
        \end{itemize}
		\item<3-> Integração do módulo de descoberta de serviços
	  	\item<4-> \emph{Spotter} acede directamente ao hardware  % Use Next Page to go to Point 3
        \end{itemize}
 \end{columns}
\end{frame}
\note{Speak clearly}  % Add notes to yourself that will be displayed when
...
\begin{frame}
  ...
  \begin{columns}
   ...
   \column{.6\textwidth}
        \begin{itemize}\setlength{\itemsep}{6mm}
               %\item<1-> Exemplo de topologia física de rede  %(Click ``Next Page'' to see Point 2) % Use Next Page to go to Point 2
	  	\item<1-> Funciona como um \emph{add-on} à plataforma
	  	\item<2-> Fornece uma \emph{API} para que outras aplicações possam usar o módulo  % Use Next Page to go to Point 3
	  	\item<3-> Funcionamento baseado no protocolo \emph{GSD}
        \end{itemize}
   \end{columns}
\end{frame}
\note{Speak clearly}  % Add notes to yourself that will be displayed when
...
\begin{frame}
  ...
  \begin{columns}
   ...
   \column{.5\textwidth}
        \begin{itemize}\setlength{\itemsep}{6mm}
               %\item<1-> Exemplo de topologia física de rede  %(Click ``Next Page'' to see Point 2) % Use Next Page to go to Point 2
	  	\item<1-> Sistema de \emph{plugins}
	  	\item<2-> Síncronos ou assíncronos
	  	\item<3-> Informação abstraída para tipo de dados comum
		\item<4-> Utiliza a \emph{API} para descobrir o respectivo \emph{manager}
        \end{itemize}
 \end{columns}
\end{frame}
\note{Speak clearly}  % Add notes to yourself that will be displayed when
...
%5	  	\item<2-> Células de dimensão variável
...
\begin{frame}
  ...
  \item<1-> Feixes de infra-vermelhos para detectar entradas e saídas de pessoas
	\begin{itemize}
  		\item<2-> Sensor de curto alcance que é simultaneamente emissor e receptor
		\item<2-> Par emissor e receptor infra-vermelho de longo alcance
		\item<3-> Uso de dois feixes em paralelo para detectar o sentido
%		\item<4-> Dispositivos de longo alcance usados de forma síncrona para não causar interferências
	\end{itemize}
  \item<5-> Tecnologia \emph{Bluetooth} para detectar a força de sinal de aparelhos pessoais 
	\begin{itemize}
  		\item<6-> Transformar força de sinal em distância ao nó
		\item<7-> Através de leis de propagação
		\item<7-> Através de \emph{fingerprinting}
	\end{itemize}
  \end{itemize}
\end{frame}
\note{Speak clearly}  % Add notes to yourself that will be displayed when
...
\tiny
\begin{center}
								Nó com integração de sensores infra-vermelho de curto alcance
	\end{center}
\normalsize
...
\begin{frame}
  \MyLogo
  \frametitle{Protótipo}\framesubtitle{Hardware} 
	\begin{center}
        \pgfuseimage{ir_emitters}
	\end{center}
\tiny
\begin{center}
								Emissores de infra-vermelho de longo alcance em paralelo
	\end{center}
\normalsize
\end{frame}
...
\tiny
\begin{center}
Nó com integração de sensores receptores de infra-vermelho de longo alcance e \emph{Bluetooth}
	\end{center}
\normalsize
\end{frame}
...
\begin{frame}
  ...
  \frametitle{Protótipo}\framesubtitle{Hardware} 
	\begin{center}
        \pgfuseimage{ir_entrance1}
	\end{center}
\tiny
\begin{center}
								Sensores de infra-vermelho de longo alcance à entrada de uma divisão
	\end{center}
\normalsize
\end{frame}
...
\tiny
\begin{center}
								Nó com acesso a tecnologia \emph{Bluetooth}
	\end{center}
\normalsize
\end{frame}
\note{Speak clearly}  % Add notes to yourself that will be displayed when
  ...
  \frametitle{Resultados}\framesubtitle{Ambiente de Testes}   % Insert frame title between curly braces
   ...
   \column{.5\textwidth}
	\begin{itemize}\setlength{\itemsep}{4mm}
               %\item<1-> Exemplo de topologia física de rede  %(Click ``Next Page'' to see Point 2) % Use Next Page to go to Point 2
		\item<1-> Realizados no núcleo 11
...
\emph{Spotters}
	\begin{itemize}\setlength{\itemsep}{4mm}
               %\item<1-> Exemplo de topologia física de rede  %(Click ``Next Page'' to see Point 2) % Use Next Page to go to Point 2
	  	\item<3-> \emph{A} e \emph{D} detectam entradas e saídas através de \emph{IV}
		\item<4-> \emph{A}, \emph{B} e \emph{C} detectam distâncias a pessoas através de \emph{Bluetooth}
        \end{itemize}
   \column{.5\textwidth}
       \pgfuseimage{planta11}        
 \end{columns}
...
\emph{Zone Managers}
	\begin{itemize}\setlength{\itemsep}{4mm}
               %\item<1-> Exemplo de topologia física de rede  %(Click ``Next Page'' to see Point 2) % Use Next Page to go to Point 2
	  	\item<3-> \emph{D} é responsável pela zona do gabinete (Contagem)
		\item<4-> \emph{E} é responsável pela zona do núcleo (Contagem e Localização)
        \end{itemize}
\vspace{3mm}
\emph{Aggregator}
	\begin{itemize}\setlength{\itemsep}{4mm}
 	\item \emph{E} agrega a informação das duas zonas
 \end{itemize}
   \column{.5\textwidth}
       \pgfuseimage{planta11}        
 \end{columns}
...
\hline
	...
	Passagem cruzada & Usualmente detecta apenas um dos sentidos \\ \hline
...
\begin{frame}
  \MyLogo
  \frametitle{Resultados}\framesubtitle{Detecção de Movimento}   % Insert frame title between curly braces
	\begin{columns}
   \column{.5\textwidth}
	\begin{itemize}\setlength{\itemsep}{4mm}
               %\item<1-> Exemplo de topologia física de rede  %(Click ``Next Page'' to see Point 2) % Use Next Page to go to Point 2
	  	\item<1-> Movimento real face ao detectado
...
\begin{frame}
  ...
  \begin{itemize}\setlength{\itemsep}{6mm}
	  \item<1-> Criação de plataforma de contagem, localização e detecção de movimento
	  ...
	  \item<4-> Adição de um módulo de descoberta de serviços à plataforma multi-funcional
  \end{itemize}
\end{frame}
\note{The end}       % Add notes to yourself that will be displayed when
		     % typeset with the notes or notesonly class options
...
\begin{frame}
  ...
  \begin{itemize}\setlength{\itemsep}{7mm}
	  \item<1-> Estudar desempenho da plataforma quanto à escalabilidade, precisão e exactidão
	  \item<2-> Implementar e aplicar o servidor de detecção de padrões de movimento
	  \item<3-> Carregar mapas dinamicamente à medida que são necessários
  \end{itemize}
\end{frame}
\note{Speak clearly}  % Add notes to yourself that will be displayed when
                      % typeset with the notes or notesonly class options
