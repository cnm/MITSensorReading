...
\begin{frame}
  ...
  \begin{columns}
   ...
   \column{.5\textwidth}
        ...
        \end{itemize}
		\item<4-> Integração do módulo de descoberta de serviços
        \end{itemize}
 \end{columns}
\end{frame}
\note{Speak clearly}  % Add notes to yourself that will be displayed when
...
\tiny
\begin{center}
Nó com integração de sensores receptores de infra-vermelho de longo alcance e Bluetooth
	\end{center}
\normalsize
\end{frame}
...
\tiny
\begin{center}
								Nó com acesso a tecnologia Bluetooth
	\end{center}
\normalsize
\end{frame}
\note{Speak clearly}  % Add notes to yourself that will be displayed when
...
Spotters
	\begin{itemize}\setlength{\itemsep}{4mm}
               %\item<1-> Exemplo de topologia física de rede  %(Click ``Next Page'' to see Point 2) % Use Next Page to go to Point 2
	  	\item<3-> \emph{A} e \emph{D} detectam entradas e saídas através de IV
		\item<4-> \emph{A}, \emph{B} e \emph{C} detectam distâncias a pessoas através de Bluetooth
        \end{itemize}
   \column{.5\textwidth}
       \pgfuseimage{planta11}        
 \end{columns}
...
\begin{frame}
  ...
  \begin{itemize}\setlength{\itemsep}{7mm}
	  ...
	  \item<3-> Carregar mapas dinâmicamente à medida que são necessários
  \end{itemize}
\end{frame}
\note{Speak clearly}  % Add notes to yourself that will be displayed when
                      % typeset with the notes or notesonly class options
